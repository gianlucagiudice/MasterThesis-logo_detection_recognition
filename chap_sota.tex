...
With the rapid development in deep learning, more powerful techniques are used to address the problems existing in traditional models \cite{zhao2019object}. 
...
\begin{figure}
    ...
    \end{center}
    \caption{Pipeline of YOLO: it divides the image into an $S \times S$ grid and for each grid cell predicts $B$ bounding boxes, confidence for those boxes, and $C$ class probabilities. These predictions are encoded as an $S \times S \times (B * 5 + C)$ tensor \cite{redmon2016you}.}
...
The parameters $\textbf{e}_l$ which define the masks are learned during training. During an epoch, a linear annealing schedule is applied to $s$ as follows:
\begin{equation}
...
Weight aligning (WA) proposed in \cite{zhao2020maintaining} is a method to alleviate catastrophic forgetting in Deep Neural Networks (DNN). In their work the authors show how a model trained for a CIL task tends
