...
Using these techniques, the performance at each task is higher than before, as shown in \autoref{fig:exp2}. Analyzing \autoref{fig:exp2-train_val}, the accuracy on the training and validation set at task 7, the training accuracy is lower than in the models without regularization, but the validation accuracy is higher. 
This is a sign that the regularization works as intended.

As shown in \autoref{table:exp2}, the top-1 accuracy of the best regularized model adopting data augmentation is 7\% higher than the one without regularization and data augmentation.
...
Performance comparisons between CIL models and baselines are carried out considering the top-1 accuracy on the test set. 
The results are shown in \autoref{table:baseline1}.
As we can see, the best CIL model performs better than the best baseline, which is the opposite of what is expected.
However, as we can see from \autoref{table:baseline1-params}, this is due to the difference in the number of model parameters described in \autoref{sec:method-baseline1}.
For this reason, to achieve a more reliable comparison, a new baseline will be used later on (see \autoref{sec:exp-baseline2}).
...
\hline 
    \end{tabular}}
    \caption{Performance comparison between CIL models and the type 1 baseline. Top-1 accuracy at task 7 (CIL model) and task 0 (baseline).}
...
\hline 
    \end{tabular}}
    \caption{Number of model parameters between CIL models and the type 1 baseline. Number of model parameters at task 7 (CIL model) and task 0 (baseline).}
...
\begin{figure}[H]
	...
	\caption{Comparison of models trained on 100 randomly sampled classed and top-100 classes considering top-1 and top-5 at each task.}%
	\label{fig:exp3}%
\end{figure}

\newpage
\subsubsection{Introduction of Adam optimizer}
The next experiments aim to compare SGD (see \autoref{sec:sgd_opt}) and Adam (see \autoref{sec:adam_opt}) optimizers. Considering regularized models, data augmentation and top-100 classes, the results of this comparison is shown in \autoref{fig:exp4} and \autoref{table:exp4}.

As we can see from the results shown in \autoref{table:exp4}, the top-1 accuracy at task 7 improves by more than 6\% using Adam as the optimization algorithm. 
...
Considering the top-1 and top-5 accuracy on the test set, the performance of CIL models and type 2 baselines is compared in \autoref{table:baseline2}.
...
Although this new baseline mitigates the problem of the number of model parameters, this issue is still present, as shown in \autoref{table:baseline2-params}.
...
\hline 
    \end{tabular}}
    \caption{Performance comparison between CIL models and type 2 baselines. Top-1 accuracy tested on 100 classes.}
...
The type 3 baseline aims to solve the problem of the number of model parameters. In fact, this approach, described in \autoref{sec:method-baseline3}, defines the baseline architecture using the DER algorithm in the same setup as the CIL classifier, thus having an identical architecture for both the CIL model and the baseline.
This ensures that the number of model parameters is the same.
The baseline is then trained using the same dataset, data augmentation, regularization, and the Adam optimizer.

The performance between CIL models and this new baseline is compared considering the top-1 accuracy on the test set. As we can see from \autoref{table:baseline3}, the type 3 baseline achieves better performance than the CIL model.
...
The results of pruning is shown in \autoref{fig:exp5-params} and \autoref{table:exp5-params}\todo{Cambiare riferimento}.
...
As we can see, the models scale fairly well over the entire dataset, as a comparison, considering the setup of 100 classes shown in \autoref{table:exp5}, the drop in performance is only about 6\%.%, even though the examples stored in the setup of 2993 classes are 1/10th of what they were before.
...
CIL\_2993-mem50-drop0.5-adam $\clubsuit$ &0.1&50&\textbf{89.22}&\textbf{92.97}\\
        \hline
    \end{tabular}}
    \caption{Performance of the models at each incremental task trained on the entire dataset using different dimensions of memory size. Top-1 and top-5 accuracy at task 8.}
    \label{table:exp6}
\end{table}
...
Similar to the setup with 100 classes, the pruning strategy is tested for models trained on the entire dataset. As we can see from \autoref{fig:exp7} and \autoref{table:exp7}, when using the pruning in combination with Weight Aligning (WA), performance drops dramatically.
Therefore, only for models using pruning, WA is disabled.

In contrast to the 100-class experiments, pruning now results in poor performance. In fact, the drop in accuracy is considerable compared to those models without pruning. Even considering the case using 50 samples for the pruning model, performance deteriorates by almost 20\%.
...
For this reason, the KD is used instead. The results of the experiments is shown in \autoref{sec:exp-kd}.
...
From \autoref{table:exp-det_100}, we can see how the detector trained on 30 classes does not generalize well on the remaining 70.
...
As in the case of the setup of the top-100 classes, the detector is trained on the first 1000 classes of the initial task, then the evaluation metrics are computed on the test set of these 1000 classes combined with the remaining 1993 classes, thus on the entire dataset consisting of 2993 classes in total.
